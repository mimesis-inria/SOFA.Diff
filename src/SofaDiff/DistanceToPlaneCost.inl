...
******************************************************************************/
#pragma once
#include <SofaDiff/DistanceToPlaneCost.h>
...
template <class TIn>
DistanceToPlaneCost<TIn>::DistanceToPlaneCost()
: Inherit()
...
template <class TIn>
void DistanceToPlaneCost<TIn>::init()
{
    Inherit::init();
...
template <class TIn>
void DistanceToPlaneCost<TIn>::apply(const core::MechanicalParams *mparams, Data<VecCoord_t<TOut>>& out, const Data<VecCoord_t<TIn>>& in)
{
    SOFA_UNUSED(mparams);
...
template <class TIn>
void DistanceToPlaneCost<TIn>::applyJ(const core::MechanicalParams *mparams, Data<VecDeriv_t<TOut>>& out, const Data<VecDeriv_t<TIn>>& in)
{
    SOFA_UNUSED(mparams);
...
template <class TIn>
void DistanceToPlaneCost<TIn>::applyJT(const core::ConstraintParams *cparams, Data<MatrixDeriv_t<TIn>>& out, const Data<MatrixDeriv_t<TOut>>& in)
{
    SOFA_UNUSED(cparams);
...
template <class TIn>
const linearalgebra::BaseMatrix* DistanceToPlaneCost<TIn>::getJ()
{
    return &J;
}

template <class TIn>
void DistanceToPlaneCost<TIn>::handleTopologyChange()
{
    if (this-> d_componentState.getValue() != sofa::core::objectmodel::ComponentState::Valid)
        return;
