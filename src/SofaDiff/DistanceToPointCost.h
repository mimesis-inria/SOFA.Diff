...
template <class TIn>
class DistanceToPointCost : public CostFunctionMapping<TIn>
{
public:
    SOFA_CLASS(SOFA_TEMPLATE(DistanceToPointCost,TIn), SOFA_TEMPLATE(CostFunctionMapping, TIn));
    typedef CostFunctionMapping<TIn> Inherit;
...
protected:
    ...
    DistanceToPointCost();
    virtual ~DistanceToPointCost() {};

    linearalgebra::EigenSparseMatrix<TIn, TOut> J;

private:
    bool m_isInFront;
    type::Vec<Deriv_t<TIn>::spatial_dimensions,typename Deriv_t<TIn>::value_type> m_difference;
};
