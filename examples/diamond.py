def createScene(root):
    ...
    with root.addChild('robot') as robot:
        ...
        with robot.addChild("Target") as target:
            target.addObject("MechanicalObject", name="Target", position=[[0, 0, 125]], showObject="true", drawMode="1", showObjectScale="1", showColor="1 0 0 1")
            target.addObject("BarycentricMapping", mapForces=False, mapMasses=False)
    ...
    with root.addChild("Loss") as loss:
        with loss.addChild("Distance") as distance:
            distance.addObject("MechanicalObject", template="Vec1d", name="output", position=0)
            distance.addObject("DistanceFromTargetMapping", input="@/robot/Target", targetPositions="@/Goal/goalMO.position", output="@/Loss/Distance/output")
